...
# CI test job) does not have it. The eac engine api imports the executor at
# module top, which imports safe_eval -> simpleeval, so guard the whole module
# so it skips cleanly instead of erroring during collection.
