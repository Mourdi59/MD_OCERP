...
# whoever that row names. Feeding them a synthetic payload finds no row, so
# they return early and the control cannot fire - not because they are silent
# but because this instrument has no database. Their equivalent of this test
# has to seed one and live in ``tests/pg``.
#
...
# removed. Each was confirmed session-dependent by reading the body, not
...
_RESOLVES_RECIPIENTS_FROM_THE_DATABASE: dict[str, str] = {
    ...
    "_wave5_cross_module_subscribers._on_qms_ncr_mirrored_from_hse": "loads the mirrored NCR",
...
async def _recipients_resolved(module, handler, event_name: str, keys: set[str]) -> int:
    """Run the handler against a payload of exactly *keys*; count notifications sent."""
    sent: list[Any] = []
    ...
    try:
        ...
        try:
            ...
            # A handler that raises resolved no recipient, which is the verdict
            # we are after. The bus isolates handlers the same way.
            pass
    finally:
        for patch in patches:
            patch.stop()
...
_VERDICT_CACHE: tuple[dict[str, str], list[str], list[str]] | None = None


async def _verdicts() -> tuple[dict[str, str], list[str], list[str]]:
    """Return (verdict per handler, opaque-publisher labels, unobservable labels).

    Memoised: the sweep executes every handler twice and the four assertions
    ...
    for event_name, handler, label in _registered_notification_handlers():
        ...
        control = await _recipients_resolved(module, handler, event_name, reads)
        if control == 0:
            unobservable.append(f"{label} ({event_name})")
            continue
        real = await _recipients_resolved(module, handler, event_name, reads & published[event_name])
    ...
    _VERDICT_CACHE = (verdicts, opaque_labels, unobservable)
    return _VERDICT_CACHE
...
async def test_every_decidable_handler_resolves_a_recipient() -> None:
    ...
    verdicts, opaque_labels, unobservable = await _verdicts()
...
async def test_the_starved_list_only_shrinks() -> None:
    ...
    verdicts, _opaque, _unobservable = await _verdicts()
...
async def test_a_handler_that_notifies_nobody_is_declared() -> None:
    ...
    """
    _verdicts_, _opaque, unobservable = await _verdicts()
