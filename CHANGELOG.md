...
Every register says how much of itself the reader is seeing. A list that would not fit used to look complete, and the answer is not a message on one screen but a rule across the product: the count comes from the register rather than from the page, a filter cannot deny what the register holds, an empty screen is no longer reported as an empty register, and a guard runs at commit time so a new list cannot ship silently truncated. Around thirty registers gained the notice, among them the quality registers, correspondence, inspections, the resource roster, the diary, contracts, the snag list and the risk section.
...
Language reaches further in and stops leaking. Forty one locales carry the product, module manifests introduce themselves in the reader's language rather than in English, page headings and the browser tab follow the language the rest of the screen speaks, and the first paint waits for the active bundle instead of showing English for a moment. Numbers are decided by one locale rather than by whichever language the page happened to open in, so percentages, compact money and measurements are written the way the reader writes them. Ukrainian and American English joined the repository. Uzbek is in the tree and deliberately not in the picker, because roughly a fifth of it still reads as English, and a language offered half translated is worse than a language not yet offered.
