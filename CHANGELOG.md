...
Names reach a document as text rather than as markup. A party name containing an ampersand or an angle bracket was passed through a layout engine that reads a small set of tags, so a company whose name carries an ampersand appeared with the escape visible, and a name that happened to contain something tag shaped could take the rest of the cell with it. Seven invoice cells were printing their own bold tags. Separately, a name too wide for its column ran across the page instead of being clipped at the column edge. Names like these are ordinary in most of the world and unavoidable in some of it, so the fix is at the point where text is handed to the layout rather than at each call site.

A document now says which language it is in. Twelve exports were letting the request middleware name the language of the file, which is the language of whoever asked for it rather than the language it was written in, so a Brazilian invoice downloaded by a German speaking user was labelled German. The regulator report carries the language it was drafted in. A document that cannot establish its own language says so instead of claiming one.

Working calendars are now the calendars of the countries they name. Canada was using American public holidays. Saudi Arabia was shipping a four day working week. The United Arab Emirates was not working Monday to Friday. Every Gulf country shared one holiday set rather than having its own. The Chinese calendar held the previous year's lunisolar festivals, which move every year, and now carries a table that checks its own arithmetic. A holiday computation that fails now says it failed instead of reading as a year with no holidays, which is the difference between an error and a year in which every day is a working day. Bulgaria and Nigeria have seeded calendars, and the working week outlives the last year the seed file covers rather than falling back to a default nobody chose.

Money that was a blend of two currencies is now either labelled or withheld. A stock balance rolls a weighted average cost forward across receipts, and receiving the same item against a euro order and then a dollar order averaged the two into a number that is money in neither, which then spread onto every issue and stocktake behind it. That average now carries the currency it is denominated in, and where the balance received more than one it says so and withholds the number rather than presenting a blend as a price. The three way match refuses an invoice it cannot compare against its order instead of subtracting one currency from another, and says so in the recipient's own language. A tolerance profile can now say which currency its absolute price floor is written in, and where nothing says it the floor is dropped rather than applied. That floor is picked by name and used against orders in every currency a tenant trades in, and the two directions of the error are not alike: one too small for the order's currency never won and changed nothing, while one too large widened the band, so the invoice was matched, approved for payment and recorded with no exception raised and nothing for anyone to read. Dropping it narrows the band instead, which is the safe direction to be wrong in, and the match result now says which band decided it. Bid parity analytics, the change order impact preview, the sales forecast and the pipeline dashboard all publish the currencies they computed with. A total says how much of it was never converted. A quote for a date with no exchange rate is refused rather than priced at zero.

A budget line reads back the same whichever way you ask for it. Creating a budget returned the number in the shape it was typed and every later read returned it at the column's own scale, so one row had two shapes depending on which request you happened to make.

An upgrade keeps the constraints it declares. What runs when this product starts is not the migration chain; it is a repair pass that compares the live schema against the model and adds whatever is missing. That pass would only mark a column as required when it could also supply a value for the rows already in the table, and it looked for that value in one of the two places a model can declare one. A column that declared its default in the other place arrived on an upgraded database as optional, and the database and the model then disagreed about it indefinitely, while every health signal reported a clean upgrade because the column existed and could be read. Just under twelve hundred columns are affected, spread across a little over half the tables in the schema, on the upgrade path only. A fresh install was never wrong.

The public demo can be left running. It refuses a websocket it has not allowlisted rather than only the requests it recognises, it can forget the files strangers upload to it, and it stores the same file once rather than once per project. Its seeders fill the projects the demo is curated around instead of whichever three came first, a tender with three bids is presented as a comparison rather than as a buyout, and the Toronto structural tender has the five bidders it says it has. A binary frame no longer drops a live socket, a presence frame no longer reports an unread lock as free, and a socket that cannot judge a token stops reporting that the token was bad.

The bundled demo ships its site photographs again. The twelve pictures the demo gallery is built around were being dropped from the published package while their captions were kept, so a fresh install seeded twelve captions with nothing behind them and every gallery came up empty. This affected installs from the package only, and it goes back several releases.

Elsewhere in this release: payment clocks for Ontario, India, Bulgaria and Nigeria, and a jurisdiction with no clock can name which of three reasons applies rather than being silently absent; bill column presets for China and Canada, and the Chinese fee stack is filed the way Chinese practice files it; a Nigerian VAT rate; a property tax quote that says how it arrived at its figure, and a table that says whether a missing VAT block is the law or a gap in our data; the double booking gap between a lead and a reservation is closed, and a plot released from a sale that fell through returns to the pool; a change order whose scenario you publish now stores one, where that endpoint had failed for every caller since it shipped; the estimate class rung is named in the reader's language; a labour share is measured against the rate rather than against the buildup, and a per unit norm stays per unit however large the position is; the recurring schedules tab opens on a register instead of on nothing; a websocket only module is visible to the loader again; the phone and address validation rules say whose rules they are; the China pack stops naming standards it does not enforce; and the ten font files this product ships are attributed by the same pipeline that publishes every other dependency, having been invisible to it because a font committed into a repository is a file rather than a package.

Two notes before you upgrade. A vendor invoice denominated in a different currency from its purchase order is now refused by the three way match rather than compared, so anything automated against that path will see refusals where it saw silent arithmetic. The invoice may be perfectly correct; what the platform declines to do is subtract one currency from another. And the schema repair described above will add constraints to an upgraded database that were previously left off. It only ever tightens what the model already declares, and where the database refuses a constraint the column is still added without it rather than skipped, but it is a change to a database that has been running.

## [15.5.0] - 2026-08-23

Approving a change order on a project that holds more than one unlocked bill of quantities now refuses and says which bills it could have meant, instead of reporting success and writing the approved scope into no bill at all. The approval moved the budget, stamped the order as approved, and returned without complaint, while the lines it was supposed to create were never created. Nothing in the response said so, and there was no way to notice except by opening the bill and finding it unchanged. That silence was worse than the guess it had replaced, because approving is idempotent: once an order is approved there is no request left that could place the scope afterwards, so a user who spotted the missing lines had nothing they could do about it. The refusal now happens before anything is written, ahead of both the status change and the notification, since an event is published outside the transaction and a refusal raised after it would leave the notification sent while the rows rolled back. A project with no unlocked bill at all still approves, because there the caller has no answer to give and plenty of projects run change control without ever opening a bill. What is refused is exactly the set a caller can do something about, and the screen that asks the question gained the control to answer it.

The same guess was being made in three other places, and it was worse than a coin toss. Where a project held more than one bill, the code took the oldest one. On a project carrying an approved estimate beside a working copy, the oldest is usually the approved one, so the rule reliably picked the bill nobody would have picked, and it did not consult whether that bill was locked. Repriced rates could therefore be written into a signed estimate. All four sites now share one rule that either resolves to a single unambiguous bill or refuses and names the candidates, and the lock is part of the question rather than an afterthought. The quantity map, which is where this is most often hit, gained a picker that appears only when there is genuinely something to choose, listing unlocked bills only, because offering a locked one would be offering a refusal.

A variation that has been promoted into a change order is added to the contract once, whichever half is approved first. Two independent parts of the system add money to a contract total, one following the variation and one following the change order, and each checked only its own record of what it had already added, so neither could see the other's work. On a contract of a hundred thousand carrying one change of twenty five thousand, the total read a hundred and fifty thousand. The first repair taught the two paths to recognise each other, and it worked in one direction only: the check asked whether the money had already arrived, and a check written on whether money has arrived is decided by who goes first. Approving the mirrored order before the variation completed found an untouched contract, posted, and let the variation post the same amount again. Both halves now compute the same name for the commercial change itself, and the contract records which sources it has already paid for, which is the shape the cost spine and the purchase order markers on that same field already used. Either ordering lands the amount once. Recording is not paying, and that distinction is preserved: a variation raised in a currency the contract does not use appears on the contract's list of variations before the currency check refuses it, so that list says the variation was seen rather than that it was paid, and anything the currency check turned away is excluded from the record of what has been posted.

A goods receipt delivered twice is no longer received twice. Confirming a receipt moves money out of committed, into actual, and writes the running total into the budget row's own record, and none of the three asked whether this receipt had already been handled. A redelivered event ran all three again, so committed fell to two hundred instead of six hundred, actual climbed to eight hundred instead of four hundred, and the record agreed with the wrong figure rather than contradicting it. The receipt is now marked on the budget row before any of the writes and stamped alongside them, so a redelivery cannot land one and skip another, and two genuinely different receipts still both land. An event arriving without a usable receipt identifier used to be the one case that returned silently, which trades a double post for a quiet refusal to record real money; it now says so in the log before returning.

Expired insurance no longer passes an award. The check that decides whether a subcontractor may be awarded work read an administrative blocked flag and a prequalification status, and neither of those is a date, so the one fact that lapses on its own was the one fact the check could not see. The expiry was recorded in the register, it was shown on the vendor page, and the award went through anyway. It now reads document expiry as well, on the day the decision is actually being taken rather than on whatever day the server happens to be having, so a decision recorded against a past date is judged against that date. A certificate expiring on the thirtieth is valid on the thirtieth and refused from the first, which is what every other part of the module already believed. Only the document types that block a payment can block an award, so an expired quality certificate or bond does not stop one. A missing document is reported but does not refuse, because a prequalification that is still pending is deliberately awardable and a check that also refused on absence would stop every new supplier on their first day and be switched off within a week. The payment side keeps the stricter line and still refuses on a missing document, so the two together read as work may be awarded before the certificate arrives and may not be paid for until it does.

Four routes in the requirements module accepted a project identifier from whoever asked and did not check that the asker could reach that project. The permission they sit behind is held across a whole installation rather than granted per project, so a person who legitimately holds it in one project could reach a requirement set in another. Two of the four could read, one could overwrite a link that already existed, and one could file a new set inside another project entirely, where it would appear in that project's own list under a name its owner did not choose. All four now verify the project the same way the rest of the product does, and answer the same not found for a project that does not exist and for one the asker may not see, so the check cannot be used to learn which projects exist. Anyone running a shared installation with more than one project should take this release.

The route that files a new set was the one worth explaining, because it is the reason the other three were not enough. A review that looks for reads compares an identifier against a record that was fetched, so it finds the routes that fetch. This route fetches nothing. It takes the project from the request, writes, and returns, so there was never a record to compare against and nothing for that kind of review to notice. It was found by walking the routes themselves rather than by reading the ones that looked relevant, and the check that now covers it is enforced by a test that names every route in the module, so a route added later cannot quietly join the set that nobody checks.

Adding a requirement no longer returns a server error, and neither does adding several at once. That is the module's main way of getting anything into it, so for anyone working through the interface rather than importing a file, the module could not be filled in at all. A row that has just been inserted has never been queried, and the strategy that loads its related rows runs on a query, so the new row came back with those collections not loaded. Writing the response then read one of them, which asks the database for rows from a place that cannot wait for an answer. The handler caught the failure and reported that the requirement could not be added, which was true and told nobody anything. Nothing caught it because the tests build their rows through helpers that refresh them afterwards, and a refresh is a query, so every requirement any test had ever looked at was in the loaded state and only a row created the way the service creates one was not.

Money on screen no longer contradicts the currency it is written in. Eighteen files across cost tables, project screens, comparison rows, footers, the value summary on the dashboard and the regional settings preview had the number of decimal places written into them as a fixed two, or in a few places a fixed zero, whatever currency was in front of the reader. A currency with no minor unit gained two digits it has no coin for, a currency with three lost its third, and a summary pinned to whole units dropped the minor unit from currencies that use one. Each of these sat where a reader looks for the figure they are about to act on. They now ask the shared resolver rather than overriding it, which means the answer is decided in one place and every one of them follows it. Two were worse than the rest. The regional settings preview never asked the resolver at all, so the sample shown to explain the setting could disagree with every screen the setting governs. And the dashboard total value tile, which shows one figure per currency on a project that mixes them, capped the decimals without setting a floor, which reads as harmless and is not: a cap below what a currency asks for is silently obeyed rather than refused, so a yen total was shown with a tenth of a yen. This does not settle how many decimals the disputed currencies should get, and no stored value or calculation was touched. Documents keep their own authority for this and were not changed, so nothing here alters what an invoice says.

Building a vector index over a cost database is given the time it takes. The browser gave up after forty five seconds on work that runs for minutes, so the screen reported a failure over an index the server went on to finish, and pressing the button again started the same work a second time. Two of the calls had no client budget at all, which could hang the panel indefinitely, and one of them read an error field the server does not send, so a genuine failure arrived as an empty message. Every call now takes a budget that matches the work, and when the client still gives up first it asks the server what actually landed rather than guessing, so a success that happened is reported as one and a real failure is quoted rather than swallowed.

A validation rule that decides a project's measurement system from the country pack that claims it was registered, was enabled, and had never once run, because nothing ever wrote the value it read. It runs now. Turning it on exposed a second half: six units that the regional packs themselves declare were missing from the vocabulary the rule judges by, including the United States pack's own square and cubic feet and all four of the Russian pack's units. Because an unrecognised unit was ignored rather than flagged, a position written in metric inside an imperial bill was invisible to the very check meant to catch it. Both halves are fixed. Every surface that runs validation now assembles what it knows about a project the same way, nine call sites across seven modules where each used to assemble its own, so a rule can no longer pass in one place and fail in another on the same bill. The note published with the previous release said these surfaces already shared one resolver and could not drift apart. That was wrong. It was true of two methods inside a single service and not true of the product, and the endpoint behind the Validate button was one of the surfaces it was not true of, so the same bill could be judged one way by the button and another way by the estimate review with nothing reporting the disagreement. It is true now. A bill with no positions in it stops passing as a matter of course, because a check that found nothing to object to and a check that had nothing to look at are not the same answer. This is the rule doing what it was written to do rather than a change of policy, but the practical effect is that projects can acquire findings on the first validation run after upgrading, and more of them than a project whose units simply contradict its country.

Completing a plot handover no longer returns a server error. The service updated the handover and then read a field back off the row it had just expired, which sent it to the database from a place where it could not go, so the plot did not move to handed over and the buyer did not advance, and the last step of a sale did not complete. A test that covered this had been skipped with a note blaming the test setup, which was wrong: it was production code failing, and the skip had been hiding it.

Restoring a file from the recycle bin no longer fails. Photos, sheets, models, takeoffs, reports and markups all returned a server error and stayed in the bin; documents and drawings worked, which is why this survived, since the two kinds most people try first were the two that were fine. The restore decided which stored values to convert back by the name of a field rather than by the type of the column it was going into, and a name is not a type: ten ordinary text columns across six kinds happen to carry a name that looks like an identifier or a date, so the restore handed them something they cannot hold. The conversion now asks the column what it is.

A failed write inside a file upload stops taking the upload with it. Three writes on that path had nothing to contain a failure, so a problem in any of them left the session unusable and the upload died later at its own commit, reporting a transaction problem rather than what had gone wrong. Each is now contained where it happens. One of them changes what the database is left holding rather than only what is reported: adding a new version and retiring the previous one are now one unit, where before a retirement that failed left the new version written with the old one still marked current, so the chain had two current versions and nothing said so.

The electronic invoice clearance module can be reached. It had no control anywhere to create the registration that every later step depends on, so on a new installation the module was present, loaded and unusable. It now carries forms for the registration and for the documents themselves, built from the module's own description of its fields, and it speaks every language the product speaks.

The certified payroll module can be reached. Every route in it answered with a server error, on its first statement, because all nineteen access checks passed their arguments in an order the function does not have. Wage determinations, worker classifications and the payroll reports themselves were all unreachable, which is the entire module. Being uniformly broken is why it lasted: a module where one route in twenty fails gets reported by the person who needed that route, and a module where every route fails looks from outside like a module that was never finished, so what reaches us is that the feature is missing rather than that it is failing.

Persian has its letters back. A check that removes invisible characters was narrowed three days after it was written, but a second copy of the same check ran from the package scripts and was never narrowed, and it removed thirty five thousand two hundred and eleven zero width non joiners from the Persian translations, along with smaller numbers from Bengali and Hebrew. In Persian that character is not decoration, it is what keeps letters from joining, so its removal changes how words are written. The translations are restored, and the rule now exists in exactly one place, with a check that fails if a second copy ever appears.

Reference data that ships with the product was carrying values that are no longer true, or were never true. Canada had one tax row, the federal rate, for a country where nine of thirteen jurisdictions charge something else on top of it or instead of it, and it now carries eleven: the federal rate, the harmonised rates of the five provinces that use them, the Quebec tax that runs in parallel, the three provincial sales taxes and the Manitoba retail tax, each with the dates it applies between, so the Nova Scotia reduction that took effect in April is a change of period rather than an overwrite of history. Romania's headline rate is current. Forty two rows of the shipped commodity classification carried a code meaning something other than what the row described, four of them codes that do not exist in the standard at all, and twenty rows carried a description of our own where the standard publishes a title, so a user matching against the published list found no match for a row that was in fact correct. Two sample files taught the wrong codes to anyone who opened them to learn the format. Rates and classifications are the part of a product that looks like a fact and is quietly a claim, and these are now the published values.

The sentence that tells you how to install the search model says which of two situations you are in. There are only two, an ordinary installation where a package can be added and a packaged build where it cannot, and they ask opposite things of the reader. The screen now carries a machine readable form of that answer beside the sentence, so the sentence can be translated into every language the product speaks without the reader losing which case they are looking at, and the two cannot disagree, because all of it comes off one decision.

Three things on the desktop side, all of which only show up when something is already wrong. An upgrade can no longer try to replace the files it is running out of. An uninstall stops the database it could not previously see, which is what left a running server behind after the application was removed. And a database that refuses to start now says what to do about it, rather than reporting the internal reason it failed.

A deployment that strangers can reach can now refuse every write and explain itself. It is off by default and turning it on is an operator decision, so a self-hosted install behaves exactly as it did before and nothing here reaches it. Where it is on, a request that would change stored data is refused with a refusal that names itself, and the interface opens a dialog rather than reporting a generic failure: it says this is a public demo and the projects are there to be looked through, that installing it takes a few minutes, how to install it, and that once installed the data is yours on your own machine. Reads are untouched and signing in still works, because a demo you cannot sign into is a demo of nothing.

There are two independent layers, and the second is why the first is allowed to be imperfect. The first refuses at the endpoint, chosen by the endpoint's identity rather than by its address, because every module is mounted twice under two spellings of its name and a rule written against the address would cover one and miss the other. The second sits at the database and refuses a write the first let through, which is not route-dependent, so a route the first layer has not been taught about still cannot write. Both were then measured rather than argued: a sweep drove every mutating route in the application, three thousand two hundred and two of them, counted every row in six hundred and twenty-five tables before and after, and found the same six hundred and six rows on both sides with no route unguarded. What that establishes is coverage as the application is assembled. It does not establish how the underlying web framework behaves on a version we have not pinned, and that is left named rather than rounded up.

Elsewhere in this release: a purchase order line can name the bill position it buys, so committed cost can be read back against the estimate instead of matched by eye, and a description field that lost focus after every keystroke no longer does; a variation promoted into a change order opens that change order rather than the register it belongs to; the demo data always has something on the overdue interfaces tile, which previously came up empty about once in every eighty seven projects, and its waste report always has waste to report, which it did not when the first metered material happened to draw nothing; a database created before the formwork catalogue was renamed repairs itself on the next start, which it did not before, because the ordinary start creates tables and stamps the current version without running a single migration; the launcher health signal has a third answer for the case it cannot determine, instead of reporting a problem it has not established; the converter downloader is pinned to a fixed commit rather than following wherever a branch points on the day, and the previous setting is still honoured; the packaged build no longer invents module names it cannot import, and the check that a packaged build carries a user interface now asks a question that can be answered no; a bill row measured in one of the Chinese words for a count keeps the count someone typed, where a linked model element used to overwrite it with a volume, which affected sixty four rows across the two shipped Chinese demo packs, and those packs now cite the current Chinese measurement standard rather than an edition superseded in September 2025; a schema file we are not permitted to redistribute in modified form has been removed from the test fixtures; the runtime upload directory is no longer published in the repository; and the front page diagrams are readable in both light and dark themes.

Three notes before you upgrade. The first is about an interface rather than a screen: approving a change order on a project with more than one unlocked bill used to succeed and now returns a conflict, so anything automated against that endpoint will start seeing failures where it saw none. The old behaviour did not merely look like it worked, it reported success, which is why this is worth knowing in advance rather than discovering. There is a third, narrower than the first: an award that used to succeed against a subcontractor whose insurance or licence has lapsed will now be refused, with the document and the date named in the refusal. If you have been awarding against lapsed paperwork, this will surface it rather than introduce it. The second is about where your data lives. The command line now honours the same environment variables the server has always honoured, so if you have one of them set and were relying on the old behaviour of using the home directory regardless, the database, the cluster and the configuration will be found in the place the variable names. This is the intended fix rather than a side effect, but it is a move.

## [15.4.0] - 2026-08-22

Closing the application no longer stops the database in the middle of what it was doing. On Windows the launcher ended the backend by ending its process tree, which is an unclean stop for PostgreSQL, and it did that on every ordinary close rather than only during an upgrade. The next start then had to replay the write-ahead log before it could do anything else, and for a large database that replay is long enough to look like an application that will not open. The launcher now asks the backend to stop over a loopback endpoint that only the launcher can reach, waits for it, and falls back to ending the process tree only when that fails. On a machine that has run an earlier version the clean stop needed one more thing before it could happen at all: the database library records every process that opens the cluster and stops the server on exit only when the list it reads back names the exiting process alone, it never removes a process that was killed rather than allowed to exit, and ending the process tree is exactly that kind of killing. So every existing install already carries a list naming processes that no longer exist, and the clean stop would have been skipped on precisely the machines it was written for, and skipped silently, because from outside a skipped stop and a completed one look the same. Those entries are now dropped on the way out, and only when the process named is demonstrably gone, since removing one that is alive would stop the database underneath something still using it. The endpoint refuses anything that is not a loopback request carrying the secret the launcher was started with, compared in constant time so the secret cannot be learned by timing, and it refuses every request outright unless this is a desktop install. Installing and uninstalling stopped being violent in the same way: the installer used to end every process on the machine that shared our executable name, which reached not only a second copy of the product that had nothing to do with the install being performed but every signed in user's session on that machine, and it now stops only processes running out of the directory being installed to, asking them to close before it insists.

Starting up no longer gives up on a database that is working. The launcher measured how long the backend had been starting and abandoned it at a fixed deadline, which is the wrong question to ask of a database replaying its log, because that work takes as long as it takes and says nothing while it does. It now measures silence instead, and recovery reports its progress every fifteen seconds with a countdown, so the launcher keeps waiting while the backend is still reporting progress and gives up only after four minutes of complete silence. The twenty minute ceiling is unchanged and still catches a backend that talks forever without ever becoming healthy, it simply stopped being the usual reason for giving up. When the launcher does give up it now says which step it was on, in words, rather than reporting that the backend did not start in time.

Modules can now be installed and removed from Settings, and chosen during onboarding. The capability existed and was reachable only by someone who knew the API. Settings gains a Modules tab that lists what is installed, groups it by category, shows what depends on what before a removal takes anything else with it, and says plainly when the current account may look but not change: the control is inert rather than present and then refused, and the endpoint refuses the same account independently, so the two agree. Switching a module off takes effect in the running process rather than at the next start, which is the difference between the feature working and looking like it works. Onboarding gains a picker, grouped by the work each module does, so a new instance starts with the modules that team actually uses instead of everything. The two are deliberately different things and the wording says so: onboarding chooses what appears in your own menu, Settings installs and removes for the whole instance, and conflating them is what previously made a module look switched off while it was still running.

A geographic information system can now read the project directly. The platform serves OGC API - Features over the data it already holds, so QGIS connects to it as a data source with no export step and no copy that can go stale: project anchors, map overlays exploded into their member features, and viewpoints, each filtered by what the account may see and each declaring whether it is project scoped rather than leaving a client to guess. The basemap tile endpoint is now part of the published interface instead of being hidden from the schema. Alongside it, a non-conformity can carry the coordinates it was raised at, and raising one puts a pin on the map, which is the loop a site inspector needs: stand at the defect, record it once, and have it appear both on the map and in the register.

Two authorisation defects are closed. Seven endpoints that write the exchange rates, work calendars and tax configurations required an account to be signed in and never checked what that account was allowed to do, and those rows are global to the installation rather than belonging to a tenant, so any account of any role could change the VAT rate and the currency conversion every estimate and invoice in the install computes from. In the same file the one action that could destroy nothing, fetching published rates from the central bank, was the one that was authorised. Separately, four endpoints answered anyone who asked. Two returned absolute server paths in their diagnostics, and they now require a permission and no longer name the path at all, because a caller entitled to the answer is still not entitled to the layout of our disk. One force-ran a converter binary for any stranger who sent the request, deliberately bypassing the cache that exists to stop exactly that, while every other route in the same file was already gated. The fourth polled a background job and was the only route in its file carrying no gate of any kind: an opaque task identifier is not a permission, it made the route a capability URL by accident, and it leaked whichever way that identifier travelled. It now asks for the same three things its two siblings had asked for all along. A neighbouring listing was looked at and deliberately left open, because it only reports whether files are present and gating it was never the fix.

An invoice in a currency written without decimals now adds up. Amounts were rounded on their way onto the page but added up before they were rounded, and rounding and addition do not commute once a line carries anything below the smallest unit the currency has. So three lines of a hundred were written as a hundred each and totalled as three hundred and one, and a receiver adding up the lines it was given got a different number from the total we sent. The European standard compares those figures exactly, with no tolerance, so the document was rejected on arrival. It was not only the line totals: the tax base and the amount due were wrong the same way and for the same reason, and across a sweep of three hundred and twelve documents two thirds of the failures were in those two rules rather than in the line sum. It affected every zero-decimal currency identically, which is what identifies it as a fault in how the document is written rather than a disagreement about any one currency, and the fix reads whatever minor-unit count the currency is configured with rather than hard-coding one. Our own validator could not see any of it, because it added the figures up itself and rounded once at the end, which is not what a receiver does: a receiver rounds nothing, it adds up the strings it was handed. Now that the amounts leave us already at the smallest unit, the two arithmetics agree.

Events that other parts of the system react to are now published after the work is saved rather than while it is still being saved. A subscriber opens its own database session, and a session cannot see rows another session has not committed, so a subscriber that wrote something pointing back at the new record failed on a constraint and the failure was logged at a level nobody reads. Seeded and imported data lost rows this way without anything appearing to go wrong. The publish now waits for the commit, and the events that carry a reference to their parent were moved over first.

The embedded database's recovery no longer trusts a guess. Before deciding whether a leftover process is the cluster it is looking for, it asks that process what it is holding. The previous check read the machine-wide table of network connections, which needs administrator rights on macOS and returns nothing useful for a cluster that listens on a Unix socket rather than a port, which is how the cluster runs on both macOS and Linux. On those platforms the check silently fell through to comparing process names, and the process it was asked about is not called postgres.

Elsewhere in this release: a coordination export can be limited to the topics that were selected, and a topic can be addressed by the identifier the format itself gives it rather than only by ours; the dashboard lists answer for the project in the address bar; a defects liability entry can record which statutory regime its limitation period comes from, so the date it produces can be justified rather than asserted; a tender award record is assembled from the procedure and says what is missing rather than quietly leaving it out; the change order trail names who submitted and who approved; feet and inches are read the way a drawing writes them; uploads written before upload roots were anchored on the data directory are still readable; and the marketing site now describes the programme and schedule work in the words planners use, in all thirty-seven languages, having previously said Gantt once and nothing else.

One note for installations upgraded with pip. This is the first release whose published package carries the migration tree, so `alembic upgrade head` becomes possible on a pip installation for the first time. Start the application once before running it. Earlier packages could not record which migrations a database had already received, so a database created by one of them looks, to the migration tool, like a database that has had none of them, and running the tool first walks the whole history against tables that already exist and stops with an error. Starting the application first brings the schema up to date and records that, so the command becomes a correct no operation instead of a failure. Nothing is lost either way, because the attempt runs in a single transaction and rolls itself back, but the order saves you the failure. Installations that run from source or from the container image are already recorded and are not affected.

## [15.3.1] - 2026-08-21

The screen that appears while the application is starting now names the version it is running. Someone whose application will not open has no other way to find that out, because the About screen lives inside the application they cannot reach, and by then the installer is usually long gone from their downloads. This matters because the message shown when starting fails is deliberately general: the same sentence about the backend not starting in time is produced by a fault that was corrected several releases ago and by one that is still open, and nothing in the report told the two apart. A report could arrive complete and carefully written and still not be answerable. The version now appears on the screen itself and as the second line of the details the copy button produces, directly under the heading and above the path to the log file, and it is written from the moment the screen first appears rather than only after something has gone wrong.

An application that was starting normally is no longer given up on while it is still working. The window the launcher allowed for the whole of startup was the same length as the budget the backend allows for bringing the local database up on its own, so a database that spent that budget recovering left nothing at all for everything that follows it: migrations, loading the modules, creating the tables and preparing first-run data. That ordering was not a remote possibility. The installer stops a running copy by ending its process tree, which stops the local database without letting it close cleanly, so the first start after an upgrade is exactly the log replay the budget exists for. The application was healthy and still working, and it was reported as one that had not started, and starting it again produced the same thing because nothing had actually gone wrong to clear. The window is now twice the budget it waits on, which leaves a comfortable margin and costs nothing when a start has genuinely failed, since that is reported the moment it happens rather than when the window closes.
...
Starting the local database on Windows no longer fails on a directory the application had just made for it. The database's own setup program takes a different path when the directory it is pointed at is already there, beginning by correcting the permissions on it, and that correction is the one thing it cannot do: it re-runs itself under a restricted account so that a database is never owned by an administrator, and the account it becomes is not always allowed to change a directory an administrator created. It is now handed a path that does not exist yet, so it makes the directory, owns what it made, and touches nothing of ours. An installation that already has a database is unaffected, since that directory is there either way.

An entry that cannot be written to the activity record stops taking the work it was describing with it. A record of what happened is written in the same breath as the thing that happened, which is what makes it trustworthy, but one bad entry was ending the whole transaction rather than only itself, so a single unwritable line about a change could discard the change. The record keeps that property and no longer holds the work hostage to it.

An installation told not to fetch the search model no longer fetches it. The switch that says so was read by the part that installs the model and not by the part that loads it, so a machine on a closed network still went looking, and what the operator got was a long wait ending in a network error rather than the immediate and honest answer that this installation does not do semantic search. A model already present is still used.

An installation can be asked what state the semantic search encoder is in, and told to fetch it. The weights are a large download, so an installation may already hold them, may be fetching them now, may have tried and failed, or may have been told not to want them at all, and until now the only way to tell those four apart was to run a search and read what came back. Both answers are available directly. Asking costs nothing, because the question is answered without loading the model, which makes it safe to ask on a timer while a download runs. Starting the fetch returns straight away rather than holding the request open for the length of the transfer, and an installation that cannot download says so in the state it returns instead of through a failure.

A database carried across an upgrade keeps the moments it already recorded. The column holding when a project route was classified was created before the application declared that it stores a point in time rather than a wall clock reading, and a fresh installation has had the right kind of column for a while, so the difference could only show on a machine that had been running for longer. On those, the stored value was read back in whatever time zone the database session happened to be in, which moves a recorded moment by the offset of the server instead of leaving it where it was put. The column is now widened on start, reading what is already in it as UTC, so the times survive the change rather than drift by it. An installation that already has the right column is left alone.

Four boxes in the electronic invoice clearance panel get their background back. They named colours the palette does not define, so a warning, a success and a rejection each drew as bare text inside a border instead of the tinted block the rest of the application uses for those same three meanings.

## [15.2.0] - 2026-08-20

Installing the desktop application no longer depends on the machine reaching Microsoft. The installer carried a small downloader for the web view runtime and fetched it during setup, so a site office behind a closed network met a failure about a component rather than about the network it was actually short of. It now carries the runtime itself. Windows also stops offering two installers for one application. The .msi and the .exe put the same product in the same directory while each kept its own record of having done so, which made taking one and later the other a way to have the application listed twice, and the gates that demanded both went in the same change as the format they demanded. If the copy you have came from the .msi, remove that entry before running the .exe; your data lives outside the installed folder and an uninstall does not touch it.

A desktop application that cannot serve now says so rather than sitting there. The startup check answered two ways, ready or not yet, so a server that had started and was reporting that it could not do its job was indistinguishable from one still coming up, and the window opened on top of it. What the person got was a shell whose every action failed, which is how a problem with an installation came to look like a broken product. A backend that answers and declares itself unfit is now a third answer, and it is shown with the reason the backend gave instead of arriving much later as a timeout that explains nothing. One that has gone quiet is still read as starting or dying rather than as degraded, so a single bad reading is not held against the next one, and a probe that is accepted and then never answered no longer holds the wait open for as long as it likes.

The update notice stops sending people to a page to hunt through. It names the installer that fits the machine it is running on, because the release lists what it carries and the browser knows which computer it is. A phone is offered nothing rather than a package for a desktop it is not. Where the release has no file for that platform, and the Fedora package is allowed to be absent by design, the release page remains as the answer rather than a broken link. The check itself now goes to this installation's own server instead of to GitHub from every browser tab, so an office behind one address stops exhausting an hourly limit meant for one visitor and reading, correctly and uselessly, that there is no update.

An order raised against a bill position now commits against the money the estimate priced. The reading side of that had been complete for a long time and nothing ever wrote the link, so committed against remaining was zero on every project: four hundred cubic metres of concrete could be on order and the line for it still read as entirely uncommitted. The buyer is not asked what a cost line is, the position resolves to one. Correcting a quantity keeps the link, which is worth saying separately because an order is rebuilt from its lines when it is edited, and a version of this that only handled the first draft would have looked right and quietly emptied the report on the first correction.

Beside it, a bill position can be asked what actually happened to it, in one row: what the estimate said, what the budget planned, what is committed and contracted, what has been installed and what has been consumed from the store. A position nobody has reported on says so rather than drawing zero progress, because never reported and reported as none are different facts, and an order beyond its estimate reads as negative rather than as merely complete.

Post-calculation reads the finished job back against what it was priced at, and it now covers material as well as labour. An earned total may only be set against an actual that covers the same lines: comparing a full estimate with a partial record of what was spent produces a margin that looks like a result and is an artefact of the gap, so where the two do not cover the same ground the page says which lines are missing rather than dividing anyway. The screen also states its own name in the top bar in every language now, which it did not before, because the heading a screen reader was given had been translated while the name a sighted reader saw was the English literal.

Six modules gained the data they were always drawn against, and this is the part to read carefully, because what landed is the substance and not yet the screen. A basis of estimate can now carry the number, where it came from and how accurate it claims to be. A project roster can say who is on the job and in what capacity. A design option can be compared on time and carbon rather than on cost alone. Formwork can be recorded as a system chosen rather than only as a price. A delivery can be booked against the bill positions it carries, and a stock movement can be tied to the position that priced the material, so what left the store can be set against what was measured. All of that is reachable through the API in this version. The screens for it follow in the next one, because putting them in front of people before every language has the words for them would mean showing most of the world a list of key names instead of a form.
...
Everything else that could stop a first start went with it. The server starts somewhere it is allowed to write instead of beside its own executable, which is read only under Program Files. It no longer believes a pidfile whose number has since been handed to something else, so a machine that was shut down mid-run starts cleanly rather than waiting on a process that is not ours. It announces the local database only once something answers on it, and it asks the cluster on the address family the cluster is actually listening on, so a first run on a host with no IPv6 loopback no longer times out against a database that was up the whole time. The command line asks about the database the product really uses rather than the one it used to.

A data directory the bundled PostgreSQL cannot open is now refused before anything touches it, and the refusal says which major was found, which was expected and where it lives. It offers the routes that keep the data first and names the destructive one last, carrying its own warning, because a major version move used to arrive three retries deep as a connection error with nothing in it about versions, and reinstalling made it no better.
...
GAEB is no longer an import format with an export bolted to it. A plain X84 export is a Hauptangebot, alternate bids opt in rather than being guessed at, the Pruefdatei survives a round trip, imported sections keep their hierarchy on every screen that shows them, and umlauts survive the filename. A bill states only what it can state: the export summary stopped claiming prices for a bill that carries none, an unpriced Zuschlagsposition is a position and not an import error, and a long text with a graphic embedded in it is capped rather than allowed to bounce the position it belongs to. Typed units resolve the way the person typing them means them: a ton entered in an American bill is the short ton, and cf reaches cubic feet the way sf already reached square feet.

Markups became something an estimator can reason about. A section carries its own rate, an exception is drawn as an exception instead of being folded into the stack that contradicts it, a bond is priced off its rate card and an escalation off an index, and the contingency stays out of a base that already carries profit. Where the stack does not add up the product says so and lets the estimator proceed, because a markup engine that refuses is a markup engine people work around. One country now yields one markup stack whichever engine is asked.

A register that cannot fit on one screen now says so, and the programme to make that true everywhere has a ratchet behind it. A list that would not fit used to look complete, and the answer is not a message on one screen but a rule across the product: the count comes from the register rather than from the page, a filter cannot deny what the register holds, an empty screen is no longer reported as an empty register, and a guard runs at commit time so a new list cannot ship silently truncated. Around thirty registers gained the notice this release, among them the quality registers, correspondence, inspections, the resource roster, the diary, contracts, the snag list and the risk section. The honest figure for the rest is 484: that is how many list endpoints the guard still counts as waiting, and it is printed by the guard rather than estimated here. What is finished is the rule and the ratchet, not the sweep.

Measurement from drawings is real work now rather than a demonstration. The seeded German floor plans carry verifiable quantities and a second issue so revisions can be compared, calibration is stated in the reader's units and digits, every quantity readout goes through one locale aware formatter, and a takeoff position links to a priced line only where the unit can actually be reached. The viewer follows the document's own project instead of whichever project the switcher last remembered.

Language reaches further in and stops leaking. Forty one locales carry the product, module manifests introduce themselves in the reader's language rather than in English, page headings and the browser tab follow the language the rest of the screen speaks, and the first paint waits for the active bundle instead of showing English for a moment. Numbers stop asking the browser which language they are in. A figure written with no locale takes the operating system's language rather than the one the reader chose, so a German running an English Windows read a German page with American separators and nothing about that was visible to whoever wrote the line. The shared formatter and 218 screens behind it now name the app locale, percentages, compact money and measurements included, and a test names every call site that still asks the browser. Ukrainian and American English joined the repository. Uzbek is in the tree and deliberately not in the picker, because roughly a fifth of it still reads as English, and a language offered half translated is worse than a language not yet offered.

A first visit is 14.9 MB lighter. The service worker keeps locale catalogues out of what it downloads up front, and six of them were slipping through on a naming rule that assumed a language code is two letters. American English and the Spanish, Portuguese and Filipino variants are not, so five full catalogues and an overlay were being handed to every visitor by the very lane written to prevent exactly that.

Semantic search stops being an install time decision. The encoder downloads in the background and reports which of five states it is in, the setup wizard shows the download rather than performing it silently, and the desktop build ships the encoder it was already fetching, on the CPU build of torch rather than the CUDA one. A reindex that wrote nothing is reported as having written nothing.

An API key is honoured at the permissions it declares, a self hosted AI endpoint belongs to the user who saved it, and three company registers stopped presenting themselves as project scoped while a register scoped to a project is now read from the project the route names. Four security advisories reported against earlier versions are fixed in this tree, and the most recent of them, an AI endpoint hijacking issue reported on 2026-08-10, reaches a tagged release for the first time here.

Reliability work that has no screen of its own. Three Alembic revisions the chain pointed at and did not have were restored, a healed sequence column is numbered the way the migration numbers it, starting the app twice no longer binds every handler twice, exports carry real filenames through every download header, and the environment doctor now proves a native extra by importing it instead of locating a directory, gives the desktop build advice it can act on, and spends one line rather than a whole report on a check that could not run.
