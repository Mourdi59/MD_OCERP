...
Closing the application no longer stops the database in the middle of what it was doing. On Windows the launcher ended the backend by ending its process tree, which is an unclean stop for PostgreSQL, and it did that on every ordinary close rather than only during an upgrade. The next start then had to replay the write-ahead log before it could do anything else, and for a large database that replay is long enough to look like an application that will not open. The launcher now asks the backend to stop over a loopback endpoint that only the launcher can reach, waits for it, and falls back to ending the process tree only when that fails. The endpoint refuses anything that is not a loopback request carrying the secret the launcher was started with, compared in constant time so the secret cannot be learned by timing, and it refuses every request outright unless this is a desktop install. Installing and uninstalling stopped being violent in the same way: the installer used to end every process on the machine that shared our executable name, which reached not only a second copy of the product that had nothing to do with the install being performed but every signed in user's session on that machine, and it now stops only processes running out of the directory being installed to, asking them to close before it insists.
